    ...
    <div className="min-h-screen bg-paper dark:bg-ink-900">
      <header className="max-w-6xl mx-auto flex items-center justify-between px-6 py-6">
        <div className="flex items-center gap-2">
          <GraduationCap size={26} className="text-ink-800 dark:text-paper" />
          <span className="font-display text-xl text-ink-900 dark:text-paper">The Ledger</span>
        </div>
      ...
      {/* Full-bleed dark hero band with the SoftAurora glow behind it — deliberately
          always-dark regardless of site light/dark mode, since the aurora effect only
          reads well against a dark backdrop. */}
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 pt-16 pb-24 grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h1 className="text-4xl md:text-5xl leading-tight text-white mb-6">
              Every graduate who came before you, <span className="text-brass-400">on the record.</span>
            </h1>
            <p className="text-ink-200 text-base leading-relaxed mb-8 max-w-lg">
          ...
          <div className="rounded-sm p-8 bg-white/5 backdrop-blur-sm border border-white/10 text-paper">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {FEATURES.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="card p-6">
              ...
              <h3 className="font-display text-lg text-ink-900 dark:text-paper mb-2">{title}</h3>
      ...
      <footer className="border-t border-ink-100 dark:border-ink-700 py-6 text-center text-xs text-ink-400">
